            ...
            while True:
                ...
                try:
                    while True:
                        ...
                        print(output, end="")
                        match = re.search(
                            r"Epoch (\d+)/(\d+):\s+(\d+)%\|.*\[(\d+:\d+)<.*?loss=(\d+\.\d+), update=(\d+)",
                            output,
                        )
                        if match:
...
def create_metadata(name_project, ch_tokenizer, progress=gr.Progress()):
    ...
    error_files = []
    text_vocab_set = set()
    for line in progress.tqdm(count, total=count):
        sp_line = line.split("|")
        if len(sp_line) != 2:
            continue
...
def get_checkpoints_project(project_name, is_gradio=True):
    ...
    if os.path.isdir(path_project_ckpts):
        ...
        files_checkpoints = (
            pretrained_checkpoints + regular_checkpoints + last_checkpoint
        )
    else:
        files_checkpoints = []
